/**
 * ata_pio.c - Generic ATA PIO mode driver.
 *
...
uint8_t ATA_PIO_SoftwareReset(uint8_t bus_number) {
...
reset_wait:
	...
	if(BIT_CHECK(control_register, ATA_STATUS_BIT_BSY)) {
		printf("ATA: Waiting some more. BSY bit set.\n");
		timeout_counter++;
		goto reset_wait;
	}
...
void SetupATA_PIO() {
    ...
    /* Install handlers. */
    PICInstallIRQHandler(IRQ_PRIMARY_IDE, ATA_PIO_PrimaryInterruptHandler);
    PICInstallIRQHandler(IRQ_SECONDARY_IDE, ATA_PIO_SecondaryInterruptHandler);

    /* Enumerate PCI bus to find controllers and ports. */
	...
	if(inb(ATA_PRIMARY_PORT_STATUS) == 0xFF) {
		...
		if(ATA_PIO_DetectDevice(ATA_PRIMARY, ATA_MASTER)) {
			ata_primary_num_drives += 1;
		}
		if(ATA_PIO_DetectDevice(ATA_PRIMARY, ATA_SLAVE)) {
			ata_primary_num_drives += 1;
		}
	}
	...
	if(inb(ATA_SECONDARY_PORT_STATUS) == 0xFF) {
		...
		if(ATA_PIO_DetectDevice(ATA_SECONDARY, ATA_MASTER)) {
			ata_secondary_num_drives += 1;
		}
		if(ATA_PIO_DetectDevice(ATA_SECONDARY, ATA_SLAVE)) {
			ata_secondary_num_drives += 1;
		}
	}
	...
	if(inb(ATA_TERTIARY_PORT_STATUS) == 0xFF) {
		...
		if(ATA_PIO_DetectDevice(ATA_TERTIARY, ATA_MASTER)) {
			ata_tertiary_num_drives += 1;
		}
		if(ATA_PIO_DetectDevice(ATA_TERTIARY, ATA_SLAVE)) {
			ata_tertiary_num_drives += 1;
		}
	}
	...
	if(inb(ATA_QUATERNARY_PORT_STATUS) == 0xFF) {
		...
		if(ATA_PIO_DetectDevice(ATA_QUATERNARY, ATA_MASTER)) {
			ata_quaternary_num_drives += 1;
		}
		if(ATA_PIO_DetectDevice(ATA_QUATERNARY, ATA_SLAVE)) {
			ata_quaternary_num_drives += 1;
		}
	}
